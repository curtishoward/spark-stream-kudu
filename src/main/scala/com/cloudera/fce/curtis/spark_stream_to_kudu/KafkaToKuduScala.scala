...
object KafkaToKuduScala {
  def main(args: Array[String]) {
    ...
    windowedStream.foreachRDD { rdd =>
       ...
       val dataFrame = rdd.map(rec => (rec._2.split(",")(0).toLong,rec._2.split(",")(1).toInt)).toDF("measurement_time","number_of_vehicles")
       dataFrame.registerTempTable("traffic")
       val resultsDataFrame = sqlContext.sql("""SELECT UNIX_TIMESTAMP() * 1000 as_of_time, ROUND(AVG(number_of_vehicles), 2) avg_num_veh, 
